...
def setup_scene():
    ...
    # Enhanced lighting for better specular highlights
    glLightfv(GL_LIGHT0, GL_DIFFUSE,  (1.0, 1.0, 1.0, 1.0))
    glLightfv(GL_LIGHT0, GL_AMBIENT,  (0.15, 0.15, 0.15, 1.0))  # Reduced ambient
    glLightfv(GL_LIGHT0, GL_SPECULAR, (1.5, 1.5, 1.5, 1.0))  # Increased specular intensity
...
def draw_textured_cube(x, y, z, size, tex):
    """
    Draw a cube with KMITL texture (with transparency support and ULTRA GLOSSY material).
    """
    ...
    # Set texture wrapping to GL_REPEAT for tiling
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT)
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT)
    ...
    # ULTRA GLOSSY material - like polished plastic or ceramic
    glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, (0.8, 0.8, 0.8, 1.0))  # Slightly darker base
    glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, (1.0, 1.0, 1.0, 1.0))  # Bright white specular
    glMaterialf(GL_FRONT_AND_BACK, GL_SHININESS, 128.0)  # Maximum shininess
    glMaterialfv(GL_FRONT_AND_BACK, GL_EMISSION, (0.0, 0.0, 0.0, 1.0))  # No emission
    ...
    glTexCoord2f(2.0, 0.0); glVertex3f( s, -s,  s)
    glTexCoord2f(2.0, 2.0); glVertex3f( s,  s,  s)
    glTexCoord2f(0.0, 2.0); glVertex3f(-s,  s,  s)
